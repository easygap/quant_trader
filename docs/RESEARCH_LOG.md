...
## 2026-09-23 관찰 트랙 오버레이 재계산 — 9/17 결론 철회

9/17 표의 관찰 트랙 결론(낙폭 제어가 칼마 0.70→0.97)은 운영 트랙과 다른 조건에서 나왔다.
표는 000660(SK하이닉스)을 넣은 10종목으로 계산했고 현금에 연 3% 이자를 붙였다. 운영 트랙은
8/07부터 000660을 뺀 9종목이고 paper 계좌 현금은 이자가 없다. 슬리브도 첫날 비중을 끝까지
들고 가서, 8%p 이탈 시 재조정하는 운영 규칙과 달랐다. `tools/risk_overlay_backtest.py`를
baskets.yaml의 보유 종목·재조정 규칙·무이자 현금으로 고쳐 다시 계산했다(2021-12~2026-09-16).

- 고정 비중: 연수익률 +6.07%, 최대낙폭 −19.5%, 샤프(rf 0%) 0.45, 칼마 0.31
- 낙폭 제어(−10%에서 절반): +3.10%, −15.0%, 0.32, 0.21
- 추세 필터(200일선 아래면 절반): +4.58%, −18.1%, 0.39, 0.25

모든 오버레이가 샤프와 칼마를 낮춘다. 낙폭 제어는 최대낙폭을 4.5%p 줄이는 대신 연수익률의
절반가량(연 3%p)을 내준다. 9/17에 기록한 '권장값'은 근거가 없어졌다 — kr_diversified_hold의
오버레이는 꺼 둔 채로 두는 것이 맞다. 같은 코드로 이전 조건(10종목·현금 3%)을 돌리면 고정
+13.59%, 낙폭 제어 +10.70%에 칼마 0.78→0.77로, 개선처럼 보였던 것도 사라진다.

노출 정책 비교(P3, 같은 9종목·고정 비중):

| 주식 비중 | 연수익률 | 최대낙폭 | 샤프(rf 0%) | 상승 포착 | 하락 포착 |
|---|---:|---:|---:|---:|---:|
| 60% | +6.07% | −19.5% | 0.45 | 45% | 48% |
| 70% | +7.02% | −22.0% | 0.46 | 53% | 56% |
| 80% | +8.28% | −23.6% | 0.49 | 60% | 64% |

포착률은 하루 평균(기하평균) 수익률의 비다. 동일비중 9종목 슬리브는 어느 비중에서든 하락을
상승보다 조금 더 맞는다 — 이 기간 지수를 끌어올린 대형 반도체의 비중이 작기 때문이다. 비중을
올리면 수익과 낙폭이 거의 같이 는다. 공짜 개선은 없다.

기대치 정정: 이 설계(주식 60%, 9종목)의 2021-12 이후 실현 연수익률은 약 6%(2022 약세장과
2026 여름 급락 포함)다. 이전에 적은 '기본 13~14%'는 주식 80%·10종목 매수 후 보유, 2026-06까지의
숫자에서 하이닉스를 뺀 값이었다.

포착률 정의도 고쳤다. 국면별 복리 수익률의 비로 재면 기간이 길어질수록 한쪽으로 쏠려(위 표를
그 정의로 내면 상승 1%, 하락 96%) 몇 달 넘게 쌓인 주간 리포트 수치도 같은 방향으로 틀어진다.
`core/performance_lens.split_by_regime`의 capture를 하루 평균의 비로 바꿨다. 적립 트랙 표도 현재
코드로 다시 만들었다 — 커밋돼 있던 표는 9/22 백테스트 수정 이전 코드의 결과였다.
