...
| target-weight portfolio drawdown guard 후보군 | 높음 | canonical 검증 완료 — `target_weight_drawdown_guard` family에 exp75/rankrisk90/pdd10/floor40 기반 `tol3`, `tol4`, `tol5`, `maxnew2`, `tol3_maxnew2`, tol4 guard-only 강화 4종, rank-risk 강화 2종, sector cap 3종, correlation cap 3종, correlation rank penalty 3종을 추가하고 `--top-n 200` full sweep에서 후보 25개 비교. 업종 매핑은 FDR Sector 누락 시 KRX KIND 상장법인 목록으로 2,766개 종목을 보강한다. `rankrisk90_tol4_corrcap85`와 `sectorcap2_corrcap85`는 top-200에서 각각 기존 tol4/sectorcap2와 동일한 return +81.19%/+82.57%, raw excess +49.30%p/+50.68%p, MDD -19.33%/-19.59%, turnover/year 886.7%/909.3%였고 관측 최대 선택 상관도는 0.8147. `corrcap80`은 관측 상관도를 0.7675까지 낮췄지만 benchmark excess Sharpe<=0으로 제외. correlation rank penalty는 top-200에서 `sectorcap2_corrpen05` return +84.22%, raw excess +52.33%p, MDD -19.59%, turnover/year 912.5%, `corrpen10` return +83.50%, raw excess +51.61%p, MDD -19.96%, turnover/year 892.3%, `corrpen05` return +82.82%, raw excess +50.93%p, MDD -19.33%, turnover/year 889.9%로 canonical 대상에 포함됐다. 그러나 canonical top-20에서는 `corrpen05` benchmark excess -8.30%p, `corrpen10` -20.96%p, `sectorcap2_corrpen05` -6.28%p로 alpha가 꺾이고 MDD도 -20.25%/-20.25%/-20.22%라 모두 `paper_only`. 기존 risk-overlay 후보만 provisional 유지. 다음은 단순 상관도 hard cap/score penalty보다 월별 손실 이후 재진입 제한 또는 변동성 예산형 target selection으로 낙폭을 직접 줄이는 쪽 |
