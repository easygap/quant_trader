...
| 유동성 필터 (일평균 거래대금 기준 종목 제외) | 높음 | 부분 완료 — target-weight pilot 실행 전 평균 거래대금 대비 주문 비율 preflight 구현. backtest universe 사전 제외 필터는 별도 과제 |
...
| Target-weight readiness audit | 높음 | **완료 — `--readiness-audit`로 주문 제출/evidence 기록 없이 capped pilot 직전 상태를 JSON artifact와 Markdown 운영 리포트로 점검. clean shadow/launch readiness, active pilot auth와 cap validation, 추천 cap, 중복 session idempotency, 실행 전 position drift, 다음 조치를 함께 판정하고 shadow 수집/audit 재실행/추천 cap 승인/capped paper 실행 명령을 남긴다. cap 승인 준비가 안 된 blocker는 non-zero 종료** |
