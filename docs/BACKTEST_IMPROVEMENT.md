...
| Target-weight shadow proof | 높음 | **완료 — dry-run plan을 `shadow_bootstrap` evidence로 기록하되 `execution_backed=False`, excess=null로 유지해 promotion을 오염시키지 않음. artifact에 cap preview를 기록해 기본 pilot cap 부족(주문 수/포지션/1건 금액/총노출)을 사전 확인** |
