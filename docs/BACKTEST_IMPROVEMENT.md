...
| target-weight portfolio drawdown guard 후보군 | 높음 | canonical 검증 완료 — `target_weight_drawdown_guard` family에 exp75/rankrisk90/pdd10/floor40 기반 `tol3`, `tol4`, `tol5`, `maxnew2`, `tol3_maxnew2`, tol4 guard-only 강화 4종, rank-risk 강화 2종, sector cap 3종, correlation cap 3종, correlation rank penalty 3종, loss re-entry guard 3종을 추가하고 `--top-n 200` full sweep에서 후보 28개 비교. 업종 매핑은 FDR Sector 누락 시 KRX KIND 상장법인 목록으로 2,766개 종목을 보강한다. `rankrisk90_tol4_corrcap85`와 `sectorcap2_corrcap85`는 top-200에서 각각 기존 tol4/sectorcap2와 동일한 return +81.19%/+82.57%, raw excess +49.30%p/+50.68%p, MDD -19.33%/-19.59%, turnover/year 886.7%/909.3%였고 관측 최대 선택 상관도는 0.8147. `corrcap80`은 관측 상관도를 0.7675까지 낮췄지만 benchmark excess Sharpe<=0으로 제외. correlation rank penalty는 top-200에서 `sectorcap2_corrpen05` return +84.22%, raw excess +52.33%p, MDD -19.59%, turnover/year 912.5%, `corrpen10` return +83.50%, raw excess +51.61%p, MDD -19.96%, turnover/year 892.3%, `corrpen05` return +82.82%, raw excess +50.93%p, MDD -19.33%, turnover/year 889.9%로 canonical 대상에 포함됐지만 canonical top-20에서는 benchmark excess가 음수라 모두 `paper_only`. loss re-entry guard는 `sectorcap2_reentry4`가 return +84.68%, raw excess +52.79%p, turnover/year 793.8%로 회전율은 낮췄지만 MDD -22.34%, `reentry4` MDD -27.29%, `reentry3` MDD -35.30%로 모두 top-200 단계에서 `paper_only`; 신규 편입 차단이 손실 종목 고착을 만들 수 있어 canonical 제외. 이후 포지션 손실 감산 3종(`posloss8_frac50`, `posloss10_frac50`, `sectorcap2_posloss8_frac50`)은 research-only 로직과 회귀 테스트를 추가했지만, 현 세션 데이터 수집 소켓 차단으로 top-200 성과 검증은 보류 상태라 승격 대상이 아니다. 기존 risk-overlay 후보만 provisional 유지. 다음은 데이터 수집 가능한 환경에서 포지션 손실 감산 full sweep을 먼저 재실행한 뒤, 통과 시에만 canonical으로 좁히는 쪽 |
| target-weight 변동성 예산 목표비중 | 높음 | 구현 완료·검증 대기 — `target_allocation_mode=inverse_volatility`가 선택 종목의 목표 금액을 rolling 실현 변동성 역수로 배분하고 sleeve weight 상한을 적용한다. `target_weight_volatility_budget` family는 tol4/pdd10/floor40 계열과 sectorcap2 계열에 `volbudget60_cap35`, `volbudget90_cap35` 후보를 추가해, 실데이터 수집 가능 환경에서 MDD 0.2%p 병목을 다시 확인할 수 있게 했다. 현재는 회귀 테스트 통과 상태이며 full sweep 전에는 승격 대상이 아니다 |
