...
| target-weight churn control 후보군 | 높음 | 후보군 추가 — `target_weight_churn_relief` family가 downside rank penalty 후보에 `max_new_targets_per_rebalance`, 격월 리밸런싱, 넓은 tolerance 조합을 결합한다. 다음 검증은 `--candidate-family target_weight_churn_relief --top-n 200` full sweep으로 turnover/year와 MDD 병목 완화 여부를 확인 |
