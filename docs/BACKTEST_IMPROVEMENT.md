...
| Snapshot DB 복구 후보 패키지 | 중간 | 완료 — target-weight portfolio snapshot diagnostics가 DB persistence proof 누락으로 복구가 필요한 경우에만 artifact 기반 trade_history/positions 후보 CSV와 manifest를 생성한다. 패키지는 `candidate_only`, `db_write_enabled=false`, `requires_authoritative_confirmation=true`를 명시해 자동 DB 반영이나 artifact-only snapshot 생성으로 오용되지 않게 하며, positions 후보에서는 0주 이하 수량을 제외하고 current blockers/status 화면에 manifest, CSV 경로, 제외 종목을 함께 보여 복구 담당자가 authoritative 원장 대조를 바로 시작할 수 있다 |
