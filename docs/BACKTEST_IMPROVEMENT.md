...
| Target-weight readiness audit | 높음 | **완료 — `--readiness-audit`로 주문 제출/evidence 기록 없이 capped pilot 직전 상태를 JSON artifact와 Markdown 운영 리포트로 점검. clean shadow/launch readiness, active pilot auth와 cap validation, 추천 cap, 중복 session idempotency, 실행 전 position drift, 유동성 preflight, 비용 반영 pre-trade risk, 다음 조치를 함께 판정하고 shadow 수집/audit 재실행/추천 cap 승인/capped paper 실행 명령을 남긴다. cap 승인 준비가 안 된 blocker는 non-zero 종료** |
| Target-weight pilot enable guard | 높음 | **완료 — `tools/paper_pilot_control.py --enable`이 target-weight 후보 승인 전에 readiness audit을 재실행하고, 운영자가 요청한 cap이 현재 plan/launch readiness/유동성 preflight/비용 반영 pre-trade risk를 만족할 때만 pilot auth를 기록. 추천 cap 미충족, stale plan, audit blocker가 있으면 승인 자체를 fail-closed 차단** |
