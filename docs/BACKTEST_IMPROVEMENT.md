...
| Paper preflight/readiness 대상 옵션 충돌 차단 | 중간 | 완료 — `tools/paper_preflight.py`와 `tools/paper_launch_readiness.py`는 `--strategy`와 `--all`을 동시에 받으면 DB 초기화와 artifact 생성 전에 argparse 단계에서 실패한다. 단일 전략 점검 의도가 전체 전략 점검으로 조용히 바뀌는 운영 실수를 차단 |
