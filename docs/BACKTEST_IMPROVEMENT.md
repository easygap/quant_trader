...
| target-weight portfolio drawdown guard 후보군 | 높음 | 후보군 추가 — 운영 손실 한도 guard와 별개로 research 백테스트 안에서 직전 평가 NAV 기준 포트폴리오 drawdown trigger, 목표 노출 floor, 재진입 cooldown을 적용하는 `target_weight_drawdown_guard` family를 추가. 다음 검증은 `--candidate-family target_weight_drawdown_guard --top-n 200` full sweep으로 MDD 완화와 alpha 훼손 여부를 확인 |
