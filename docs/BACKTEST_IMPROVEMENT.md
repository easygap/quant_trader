...
| Snapshot DB 복구 후보 패키지 | 중간 | 완료 — target-weight portfolio snapshot diagnostics가 DB persistence proof 누락으로 복구가 필요한 경우에만 artifact 기반 trade_history/positions 후보 CSV와 manifest를 생성한다. 패키지는 `candidate_only`, `db_write_enabled=false`, `requires_authoritative_confirmation=true`를 명시해 자동 DB 반영이나 artifact-only snapshot 생성으로 오용되지 않게 하며, positions 후보에서는 0주 이하 수량을 제외한다. 후보 trade_history의 가격·금액·수수료·세금·슬리피지는 같은 실행 artifact의 `details[].result`와 `pre_trade_risk_check.order_costs`에서 보강하고 `price_source`로 출처를 남긴다. `--prepare-db-restore-review-bundle`은 후보 CSV 사본과 빈 reviewed authoritative CSV 템플릿, 검토 checklist/verify 명령을 no-write로 생성해 candidate row를 원장 증거로 오인하지 않게 한다. current blockers/status는 review bundle 생성 전에는 bundle 명령을, 생성 후에는 authoritative 템플릿 경로와 verify 명령, 실제 reviewed CSV 파일에서 다시 읽은 행 수/빈 템플릿/누락 컬럼, 후보 대비 누락·예상 밖 행 수, 마지막 verify 이후 CSV 변경 여부를 1순위로 노출한다. `--verify-db-restore-package`는 manifest 해시·CSV 행 수·현재 DB 상태·reviewed authoritative CSV 일치 여부를 no-write로 검증하고, 빈 reviewed 템플릿은 `authoritative_*_csv_empty_template`, 후보 CSV 파일을 authoritative 입력으로 직접 넘긴 경우는 `authoritative_*_csv_candidate_source_rejected`, 후보 전용 marker가 authoritative CSV 안에 남은 경우는 `authoritative_*_csv_candidate_marker_rejected`, 비교 컬럼 누락은 `authoritative_*_csv_columns_missing`, 행 수 불일치는 `authoritative_*_csv_row_count_mismatch`, 값 불일치는 `authoritative_*_csv_content_mismatch`로 나눠 표시한다. 값 불일치는 추가로 `authoritative_*_csv_identity_mismatch`와 `authoritative_*_csv_economic_mismatch`로 분해해 주문 식별자 문제와 가격·금액 문제를 구분한다. 최신 검증 상태와 blocker를 표시해 복구 담당자가 authoritative 원장 대조를 이어갈 수 있다 |
| Target-weight current blockers 실행 가드 | 높음 | 완료 — `run_pilot --execute`가 운영 `reports/current_blockers.json` 누락, stale freshness, 현재 전략·거래일 action 누락, DB persistence/review/snapshot recovery guard, `# blocked:` 명령을 감지하면 preflight/idempotency/position read/order submission 전에 `skipped_current_blockers_guard`로 중단한다. 운영 화면의 `# blocked` 명령을 무시하고 직접 실행 CLI를 조립하거나 오래된 current blockers를 둔 상태에서도 주문 경로로 넘어가지 않는다 |
| DB 복구 검증 stale 차단 | 높음 | 완료 — reviewed authoritative CSV가 `restore_ready=true` 검증 이후 다시 수정되면 current blockers가 수동 DB 복구 ready 액션으로 넘어가지 않고 `target_weight_authoritative_db_restore_verification_stale_after_review_edit` guard와 재검증 명령을 1순위로 노출한다 |
| DB 복구 적용 계획 | 높음 | 완료 — `--plan-db-restore-apply --restore-verification <verification_json>`이 `restore_ready=true` 검증 리포트에서 authoritative CSV sha, 후보 hash, current DB empty precondition, no-write safety를 재확인한 뒤 trade_history insert와 positions 절대값 교체용 no-write 적용 계획 JSON/Markdown을 생성한다. 검증 후 CSV가 바뀌었거나 현재 DB에 이미 trade/position이 있으면 blocked로 남기며, current blockers는 검증 완료 직후 곧장 수동 DB write를 요구하지 않고 이 적용 계획 생성 명령을 먼저 노출한다 |
| DB 복구 guarded apply | 높음 | 완료 — `--apply-db-restore --restore-apply-plan <apply_plan_json>`은 `--backup-confirmed --confirm-db-restore-apply`가 모두 있을 때만 실행되며, 적용 직전 verification/CSV/current DB 상태를 다시 재검증하고 같은 account/date의 trade_history 또는 positions가 이미 있으면 DB write 없이 blocked 처리한다. 통과 시 trade_history와 positions를 단일 DB 트랜잭션으로 반영하고 적용 결과 artifact를 남긴다. current blockers는 ready apply plan이 있으면 수동 write 대신 guarded apply 명령을 안내한다 |
