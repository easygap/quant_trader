...
| target-weight portfolio drawdown guard 후보군 | 높음 | canonical 검증 완료 — `target_weight_drawdown_guard` family에 exp75/rankrisk90/pdd10/floor40 기반 `tol3`, `tol4`, `tol5`, `maxnew2`, `tol3_maxnew2`와 tol4 guard-only 강화 4종을 추가하고 `--top-n 200` full sweep에서 후보 14개 비교. `tol4` 원형은 canonical에서 return +183.02%, benchmark excess +33.63%p, turnover/year 986.5%로 회전율은 통과했지만 MDD -20.25%로 `paper_only`. 추가 변형 중 `pdd10_floor35`는 turnover/year 829.8%이나 MDD -20.20%, `pdd8_floor35`도 MDD -20.20%로 실패했고, `pdd8_floor40`/`cooldown2`는 MDD와 turnover는 통과권이나 benchmark excess Sharpe<=0으로 신규 canonical 대상에서 제외. 기존 risk-overlay 후보만 provisional 유지. 다음은 guard-only 조정보다 downside rank penalty, 상관·업종 집중도 페널티, 비용 민감도 기준을 함께 보수화하는 검증 |
