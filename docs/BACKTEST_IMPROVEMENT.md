...
| Target-weight execution-aware evidence | 높음 | **완료 — `run_pilot(execute=True, collect_evidence=True)`는 `executed == planned`, failed=0, skipped=0, halted=false일 때만 `pilot_paper` evidence를 수집. 부분 실행/중단은 세션 artifact와 pilot session에 `target_weight_execution_incomplete` 사유를 남기고 non-zero CLI 종료로 자동화가 clean execution-backed evidence로 오인하지 못하게 차단** |
