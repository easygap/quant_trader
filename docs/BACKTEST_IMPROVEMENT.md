...
| target-weight portfolio drawdown guard 후보군 | 높음 | canonical 검증 완료 — `--candidate-family target_weight_drawdown_guard --top-n 200`에서는 2개 후보가 research 기준 provisional 조건에 들어와 `RUN_CANONICAL_EVALUATION`으로 판정. 이후 `tools/evaluate_and_promote.py --canonical`에 eligible 2개 후보를 포함해 재평가했다. pdd8/floor25 후보는 return +142.93%, Sharpe 1.43, PF 7.69, MDD -15.89%, turnover/year 530.8%였지만 canonical benchmark excess -6.46%p라 `paper_only`. exp75/rankrisk90/pdd10/floor40 후보는 return +180.40%, benchmark excess +31.01%p, Sharpe 1.49, PF 4.85, MDD -19.24%, WF positive/Sh+ 100%였지만 turnover/year 1026.3%로 `paper_only`. 기존 risk-overlay 후보만 provisional 유지. 다음은 exp75 계열에 tolerance·신규 편입 제한을 결합해 turnover를 1000%/y 아래로 낮추는 검증 |
