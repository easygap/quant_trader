...
| target-weight portfolio drawdown guard 후보군 | 높음 | canonical 검증 완료 — `target_weight_drawdown_guard` family에 exp75/rankrisk90/pdd10/floor40 기반 `tol3`, `tol4`, `tol5`, `maxnew2`, `tol3_maxnew2`, tol4 guard-only 강화 4종, rank-risk 강화 2종, sector cap 3종을 추가하고 `--top-n 200` full sweep에서 후보 19개 비교. 업종 매핑은 FDR Sector 누락 시 KRX KIND 상장법인 목록으로 2,766개 종목을 보강한다. `rankrisk120_tol4_sectorcap2`는 top-200에서 return +85.49%, raw excess +53.60%p, MDD -19.94%, turnover/year 941.5%, `rankrisk90_tol4_sectorcap2`는 return +82.57%, raw excess +50.68%p, MDD -19.59%, turnover/year 909.3%로 canonical 대상에 포함됐다. canonical 재평가에서는 `rankrisk90_tol4_sectorcap2`가 return +185.39%, benchmark excess +36.00%p, turnover/year 981.0%로 원본 tol4보다 개선됐지만 MDD -20.22%로 `paper_only`; `rankrisk120_tol4_sectorcap2`는 benchmark excess -23.70%p, MDD -20.74%로 제외. 기존 risk-overlay 후보만 provisional 유지. 다음은 단순 sector count cap보다 상관도 cap·재진입 제한·변동성 예산형 target selection 결합 검증 |
