...
| Target-weight readiness audit | 높음 | **완료 — `--readiness-audit`로 주문 제출/evidence 기록 없이 capped pilot 직전 상태를 단일 JSON artifact로 점검. clean shadow/launch readiness, active pilot auth와 cap validation, 추천 cap, 중복 session idempotency, 실행 전 position drift, 다음 조치를 함께 판정하고 cap 승인 준비가 안 된 blocker는 non-zero 종료** |
