...
| Target-weight execution-aware evidence | 높음 | **완료 — `run_pilot(execute=True, collect_evidence=True)`는 `executed == planned`, failed=0, skipped=0, halted=false이고 주문 대상 종목의 실제 paper position 수량이 각 주문의 `target_quantity`와 일치할 때만 `pilot_paper` evidence를 수집. 부분 실행/중단은 `target_weight_execution_incomplete`, 포지션 불일치는 `target_weight_position_mismatch`로 세션 artifact와 pilot session에 기록하고 non-zero CLI 종료로 자동화가 clean execution-backed evidence로 오인하지 못하게 차단** |
