...
### 자본 숫자는 결정 시점에 재계산할 것 (2026-07-06 추가)

위 표의 숫자는 작성 시점(7/2, 하이닉스 242.4만) 시세다. 7/6 시세(232.2만)로는 **32M이
34M보다 우수**(배치율 77.4% vs 74.5%, 여유 +10.2%)로 뒤집혔다 — 정수 절사의 톱니 효과
때문에 최적 자본은 시세와 함께 움직인다. 그래서 고정 숫자 대신 도구와 기준으로 결정한다:

```powershell
# 결정 시점의 실시간 시세로 자본별 채움율 재계산 (읽기 전용)
.venv\Scripts\python.exe tools/capital_slot_simulator.py
.venv\Scripts\python.exe tools/capital_slot_simulator.py --capital 32000000   # 단일 상세
```

**선택 기준**: ① 미체결 0개, ② 총 배치율이 설계 대비 -5%p 이내(헬스 ATTENTION 임계값과
동일), ③ 하이닉스 채움율 ≤ 92% (가격이 +8~10% 올라도 미체결로 안 돌아갈 여유) — 를
만족하는 **최소 자본**. live 투입 예정 실자본을 이미 안다면 그 금액이 항상 최우선이다.

### 실행 절차 (#427 도구로 준비 완료 — 가역·비파괴)

```powershell
# 1) config/baskets.yaml의 kr_diversified_hold에 한 줄 추가: initial_capital: <결정 금액>
...
**재시작일 주의 — 사이클을 하루 여러 번 돌리지 말 것**: 6/10의 진입 타이밍 비용(-1%p)은
그날 사이클이 4회 수동 실행돼(10:01~13:58) 하루에 61%를 몰아 산 것이 원인이었다. 정상
자동화(1일 1사이클)에서는 회전 상한(15%/일)이 초기 매입을 5~6거래일에 걸쳐 자연 분할한다
...
→ (2026-07-06) P0-1 머지 + 재시도 크론 설정 완료 — 재시작 전제조건 충족.
...
>   → **재시도 크론 설정 완료(2026-07-06)**: 스케줄 작업 `basket-snapshot-retry`(평일 11:00·14:00,
>   당일 스냅샷 결측 시에만 사이클 재실행 — TODAY_OK면 무동작) + `weekly-basket-report`
>   (금 16:00, --mode weekly_report). 운영 검증: 7/3·7/6 사이클에서 CYCLE_START/END·
>   SNAPSHOT_SAVED·GAP(6/26, warning) 이벤트가 설계대로 기록됨을 확인.
> - ⬜ P0-2 SMTP 재발급 — 오너 액션
...
> - ✅ **P1-4 성과 귀속 자동화** — `core/basket_evaluation.py`에 순수 `decompose_return_gap`
>   + `compute_design_portfolio_return`(fetch 주입 가능) 추가. `tools/basket_paper_evaluation.py`가
>   실제 NAV / 설계 NAV / KS11을 자동 분해(실행 격차 = 통제 가능, 구성 격차 = 설계 수용)해
>   §2의 수작업 분석을 매 실행 자동화. 종목별 시세 조회라 `include_attribution`로 CLI 전용
>   (일일 사이클 부담 없음, `--no-attribution`로 생략 가능). 실CLI 스모크 확인.
>   적대적 리뷰 반영: NAV(마지막 스냅샷 시점)와 벤치마크·설계 조회 종료일을 일치시켜
>   하루치 시장 변동이 실행 격차로 오귀속되지 않게 창을 정합.
> - ✅ **P1-7 주간 요약 리포트** — `core/weekly_report.py`에 순수 `build_weekly_summary`,
>   `main.py`에 `--mode weekly_report`. 주간 성과 + 귀속 분해(실행/구성 격차 — CLI 전용이던
>   P1-4를 오너에게 주간 푸시) + 진행률/커버리지 + 주간 이벤트를 Discord 한 장으로. 금요일
>   크론에 추가해 쓴다. 적대적 리뷰 반영: 결측은 SNAPSHOT_GAP 이벤트 원시 카운트 대신
>   '고유 일수'로 집계(재경보 부풀림 방지), 주간 변화 기준 스냅샷을 ~1주 밴드로 제한
>   (공백 시 다주간 수익 오표기 방지). 실데이터 스모크 확인.
> - ⬜ P1-6 (스냅샷 시점 정합), P2~P3 — 대기
>   - P1-6은 트랙레코드 재시작(오너 자본 결정) 시점에 함께 다루는 게 안전(운영 중 스냅샷
>     시점 변경은 시계열 일관성 훼손) — 재시작 전까지 보류.
...
| 9 | ~~분할 진입 옵션~~ → **코드 불필요로 종결** (2026-07-06) | 수익성 | 원인 재분석 결과 6/10 타이밍 비용(-1%p)은 그날 사이클 4회 수동 실행으로 하루 61% 집중 매입한 것이 원인. 정상 자동화(1일 1사이클)는 회전 상한 15%/일이 초기 매입을 5~6거래일 자연 분할한다 — 별도 기능 대신 §3 재시작 절차에 "하루 여러 번 돌리지 말 것" 명시로 대체. |
...
# 자본별 슬롯 채움 시뮬레이션 (자본 결정 지원, 실시간 시세, 읽기 전용)
.venv\Scripts\python.exe tools/capital_slot_simulator.py
# 트랙레코드 재시작 (자본 결정 후)
...
# 주간 요약 리포트 (금 16:00 크론 설정됨 — 수동 실행도 가능)
.venv\Scripts\python.exe main.py --mode weekly_report
```
