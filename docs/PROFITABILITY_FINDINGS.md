...
**좁은 유니버스(직접 고른 10~20종목)에서는 저변동성 팩터가 벤치마크를 이겼지만, 넓은
canonical 30종목에서는 그 우위가 사라졌다(NO_ALPHA_CANDIDATE).** 즉 그 "엣지"는 진짜
alpha가 아니라 **유니버스 선택 효과**였다. 공정한 넓은 유니버스로 보면 모멘텀이든 저변동성이든
동일비중 buy&hold를 꾸준히 이기는 검증된 alpha는 **아직 없다.** 지금 상태로 실거래에 자본을
넣는 것은 권장하지 않는다.
