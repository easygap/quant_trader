...
즉 **알파는 없지만 베타(시장 보유)는 충분히 좋은 수익**이다. 능동 운용으로 그걸 넘으려 하지 말고,
...
| 항목 | 값(2023~25 백테스트, 비용 반영) | 비고 |
|------|------------------------------|------|
| 실행 기준 수익률 | **+83.4%** (~22% CAGR) | 월 리밸런싱+익일시가+수수료/세금/슬리피지 |
| Sharpe | ~0.97 | 무비용 buy-once-hold는 1.24지만 실행시 하락 |
| MDD | ~−20% | |
| 회전율 | 낮음(저회전 설정) | 능동 리밸런싱은 비용이라 최소화 |
