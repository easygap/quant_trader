# 종목 선택 전략 비교 — 2026년 6월 실험 기록

> 아래 수치는 당시 데이터와 실험 조건에서 나온 과거 결과입니다. "안정적 고수익이 가능하다", "알파는 없다"는 기존의 일반화는 철회합니다. 일부 전략이 비교군에 뒤졌다는 사실만으로 미래 수익성이나 모든 능동 전략의 한계를 단정할 수 없습니다. 2026년 9월 변경과 새 검증은 [위험 관리 검증 보고서](RISK_REVIEW_20260917.md)를 보세요.

## 이 실험에서 확인한 것

2023~2025년 비교에서 대형주 30종목 동일비중 보유는 +123.8%를 기록했습니다. 여기서 시험한 모멘텀·저변동성·위험 균형·이동평균 규칙은 해당 비교군을 넘지 못했습니다. 아래의 비용·기간·유니버스 조건을 함께 읽어야 하는 결과입니다.
...
권장 운용 = **분산 대형주 저회전 buy&hold** (`config/baskets.yaml: kr_diversified_hold`).
날짜별 실현 트랙레코드: [`docs/TRACK_RECORD_buy_and_hold.md`](TRACK_RECORD_buy_and_hold.md)
(`tools/basket_track_record.py`로 재현). 핵심:

| 운용 방식 | 총수익률(2023~25) | CAGR | Sharpe | MDD | 비고 |
|-----------|------------------|------|--------|-----|------|
| **초기매수 후 보유(회전 0)** | **+132.9%** | **+32.6%** | 1.29 | −23.2% | 진입 수수료만, 가장 좋은 실행 |
| 월 리밸런싱(비용 반영) | +83.4% | ~22% | 0.97 | ~−20% | 능동 리밸런싱은 비용이라 수익↓ |

위 두 행이 결론을 그대로 보여준다: **회전을 줄일수록 수익이 좋다.** buy-once-hold가 +132.9%인데
월 리밸런싱은 +83.4%로 떨어진다. 그래서 권장 운용은 "사서 들고 있기 + 큰 이탈만 교정".
...
이 표본에서는 저회전 분산 보유의 성과가 더 좋았습니다. 다른 기간과 자산에서도 같은 결과가 나오는지는 별도의 검증이 필요합니다.
