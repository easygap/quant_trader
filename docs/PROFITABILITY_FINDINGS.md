...
## 배포 바스켓 수익의 해부 — 집중은 정확히 한 종목 (2026-06-11)

leave-one-out 분해(같은 도구·기간 2021-12~2026-06, 정수주·수수료 반영, 종목별 10회):

| 제외 종목 | CAGR | Δ기준(+27.3%) |
|---|---|---|
| **000660 SK하이닉스** | **+13.6%** | **−13.7%p** |
| 005930 삼성전자 | +26.8% | −0.5%p |
| 그 외 8종목 각각 | +27.7~+29.8% | +0.4~+2.5%p |

4.5년 +27.3% CAGR의 수익원은 사실상 **SK하이닉스 한 종목**(AI/HBM 랠리)이다. 카카오·네이버·
LG화학·포스코 4종목은 빼면 오히려 성과가 오르는 음의 기여였다.

**기대치 설정(이 문서의 결론 보정):**
- 하이닉스급 랠리 재현을 가정하지 않는 **보수적 기준선: CAGR +13~14%** — 위 robustness
  분석(2022 약세 포함 ~13%)과 독립적 방법으로 합치한다. 두 방법이 같은 숫자를 가리킨다.
- **+27%는 상방 시나리오**다. 운영·자본 결정·승격 평가에서 기본 기대는 13~14%로 둔다.
- 등가중 보유 논리는 그대로 유효하다: 다음 하이닉스를 사전에 알 수 없고(선택 알파 없음은
  이 문서의 정착 결론), 보유하지 않으면 그런 기여를 받을 가능성 자체가 0이다.
- 부수 함의: paper 트랙레코드의 자본 규모는 **실전 투입 예정 자본과 일치**해야 대표성이
  있다(정수주 단위 때문에 자본이 작으면 고가 종목 슬롯이 비는 등 구성 자체가 달라진다).
