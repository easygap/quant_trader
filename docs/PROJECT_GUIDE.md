...
| ✅ **Research candidate sweep** | `tools/research_candidate_sweep.py` — rotation/momentum/breakout/pullback/benchmark-relative/risk-budget 후보군을 benchmark-aware artifact로 랭킹하고 decision action 생성. promotion/live gate와 분리 |
...
| 현재 후보군 | rotation/momentum/breakout 기존 14개 모두 research_only 유지. pullback 4개, benchmark-relative 3개, risk-budget 5개는 신규 research-only 검증 대상 |
| 다음 후보 탐색 | risk-budget도 excess gate 실패. 다음은 방어형 exposure를 유지하면서 동일 유니버스 B&H를 직접 이길 수 있는 상대강도/현금 전환/부분 헤지 설계 우선 |
