...
| ✅ **target-weight current blockers 실행 가드 추가** | `tools/target_weight_rotation_pilot.py --execute`가 `reports/current_blockers.json`의 현재 전략·거래일 next action에 DB persistence/review/snapshot recovery guard 또는 `# blocked:` 명령이 남아 있으면 preflight/idempotency/position read/order submission 전에 `skipped_current_blockers_guard`로 중단한다. 운영 화면의 차단 명령을 우회해 직접 실행 CLI를 조립해도 DB 복구 proof 미완료 상태에서는 주문 경로로 넘어가지 않는다 |
