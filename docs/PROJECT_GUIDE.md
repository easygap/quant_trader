...
| ✅ **live gate 승격 재계산 검증 추가** | `core/live_gate.py`가 `promotion_result.json`의 status만 신뢰하지 않고 canonical metrics와 paper evidence를 `promotion_engine`에 다시 태워 `promote()` 결과를 재계산한다. `core.promotion_engine.load_metrics_from_artifact()`도 `metrics_summary.json`의 WF/benchmark 파생값이 `walk_forward_summary.json`, `benchmark_comparison.json` 원천값과 일치할 때만 metrics를 반환한다. MDD, PF, WF, turnover, benchmark, evidence 조건 중 하나라도 현재 규칙에서 `live_candidate`를 만들지 못하거나 원천 산출물이 stale이면 실전 진입을 fail-closed 차단한다 |
