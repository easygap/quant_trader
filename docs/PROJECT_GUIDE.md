...
│   ├── evaluate_and_promote.py         # Canonical 평가 → artifact → 승격 판정 + blocker summary
...
| ✅ **promotion blocker summary 추가** | `tools/evaluate_and_promote.py --canonical`이 `promotion_blocker_summary.json/md`를 함께 생성한다. 전략별 status, 주요 blocker, 핵심 metrics, 다음 운영 조치를 한 화면에 정리해 긴 `promotion_result.reason`을 직접 파싱하지 않아도 된다 |
