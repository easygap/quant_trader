...
| ✅ **target-weight 기존 증거 재사용 검증 강화** | 같은 candidate/date/params hash의 기존 `pilot_paper` evidence라도 target list, 시작 수량, 목표 수량, target exposure, gross exposure, 최대 주문금액이 현재 plan과 다르면 `already_recorded` 증거로 재사용하지 않는다 |
