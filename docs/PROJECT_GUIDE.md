...
| ✅ **target-weight paper/pilot adapter 추가** | `core/target_weight_rotation.py` + `tools/target_weight_rotation_pilot.py`로 직전 거래일 점수 기반 목표비중 plan을 만들고 pilot cap을 plan-level로 검증. `OrderExecutor.execute_buy_quantity()`로 paper-only exact quantity 매수를 지원. 실행형 `pilot_paper` evidence는 주문 제출 직전 실제 paper position이 계획 입력 장부 `position_quantities_before`와 일치하고, 계획 주문 전부 성공(`executed == planned`, failed/skipped/halted 없음) 후 실제 paper position 전체가 리밸런싱 후 `target_quantities_after` 장부와 일치하고 계획 밖 양수 포지션이 없을 때만 수집한다. 실행 전 포지션 드리프트/부분 실행/중단/포지션 불일치는 세션 artifact에 차단 사유를 남긴 뒤 승격 증거에서 제외한다. live 모드는 계속 거부 |
...
> **문서 버전**: v5.2
> **최종 수정**: 2026-05-04 (target-weight pilot position reconciliation 반영)
