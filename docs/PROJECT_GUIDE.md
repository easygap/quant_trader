...
| 다음 후보 탐색 | top-200 리스크 완화·저회전·변동성 타깃·리스크 페널티 랭킹·churn control sweep을 모두 확인한 결과, 랭킹 페널티는 raw/exposure-matched excess와 WF 안정성을 개선했지만 교체 제한을 더해도 MDD와 turnover 병목을 동시에 해소하지 못했다. 다음은 단순 리밸런싱 억제보다 포트폴리오 drawdown guard, 손실 후 재진입 cooldown, 상관/업종 집중도 페널티를 결합해 손실 구간 자체를 줄이는 후보군을 설계한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하되, top-200 후속 결과만으로 신규 paper/live 전환은 진행하지 않는다 |
