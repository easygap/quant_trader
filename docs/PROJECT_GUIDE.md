...
| 파일 | 역할 |
|------|------|
| **data_collector.py** | **`fetch_stock`**: 미국 티커(`is_us_ticker`)는 yfinance(수정주가). 한국: FDR → yfinance → KIS 폴백. **소스 추적**: `_last_source`, `_source_history`, `check_source_consistency()`. `allow_kis_fallback: false`로 KIS 일봉 폴백 차단 가능. `fetch_korean_stock`은 한국 전용 내부/레거시 호출에 사용. `get_krx_stock_list`, **`get_sector_map()`**(FDR Sector → KRX KIND 상장법인 업종 → pykrx). |
