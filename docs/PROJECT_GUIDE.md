...
| 다음 후보 탐색 | top-200 리스크 완화·저회전·변동성 타깃·리스크 페널티 랭킹·churn control·drawdown guard sweep과 canonical 손실방어 재검증을 모두 확인했다. pdd8/floor25 후보는 MDD와 turnover를 크게 낮췄지만 canonical top-20 벤치마크를 이기지 못했고, exp75/rankrisk90/pdd10/floor40 후보는 초과수익·WF 안정성·MDD는 통과권이지만 turnover/year 1026.3%로 1000%/y 게이트에 걸렸다. 다음 후보 탐색은 exp75 계열에 `target_tolerance_pct` 또는 `max_new_targets_per_rebalance`를 결합해 turnover만 낮추는 좁은 실험으로 진행한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하며, research artifact만으로 신규 paper/live 전환은 진행하지 않는다 |
