...
| **tools/research_candidate_sweep.py** | promotion/live artifact와 분리된 research-only 후보 공장. rotation 변형을 포트폴리오 단위로 평가하고 benchmark excess, EV, CAGR, turnover, WF 안정성으로 랭킹해 `reports/research_sweeps/`에 저장. |
...
| ✅ **테스트 292건 green** | live/paper/promotion/research sweep 회귀 묶음 기준 |
...
| ✅ **Research candidate sweep** | `tools/research_candidate_sweep.py` — rotation 변형을 benchmark-aware artifact로 랭킹. promotion/live gate와 분리 |
