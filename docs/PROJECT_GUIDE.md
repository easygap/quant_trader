...
| ✅ **target-weight churn control 후보군 추가** | `max_new_targets_per_rebalance`로 리밸런싱당 신규 편입 종목 수를 제한하고, downside rank penalty 후보에 격월 리밸런싱·넓은 tolerance를 결합한 `target_weight_churn_relief` family를 추가했다. 다음 top-200 full sweep에서 turnover/year와 MDD 병목 완화 여부를 검증한다 |
...
| 다음 후보 탐색 | top-200 리스크 완화·저회전·변동성 타깃·리스크 페널티 랭킹 sweep을 모두 확인한 결과, 랭킹 페널티는 raw/exposure-matched excess와 WF 안정성을 개선했지만 MDD와 turnover 병목을 동시에 해소하지 못했다. 다음은 새로 추가한 `target_weight_churn_relief` family를 `--top-n 200` full sweep으로 검증해 downside rank penalty의 alpha 후보성을 유지하면서 churn과 MDD가 완화되는지 확인한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하되, top-200 후속 결과만으로 신규 paper/live 전환은 진행하지 않는다 |
