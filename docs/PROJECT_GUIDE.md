...
| ✅ **target-weight 리스크 페널티 랭킹 후보군 추가** | `target_weight_downside_rank_relief` family를 추가했다. 기존 target-weight composite score에서 rolling downside volatility와 rolling drawdown 페널티를 차감해, 단기 모멘텀은 강하지만 최근 하방 리스크가 큰 종목이 top-N에 들어오는 경로를 줄이는지 검증한다 |
...
| 다음 후보 탐색 | top-200 리스크 완화·저회전·변동성 타깃 sweep을 모두 확인한 결과, 단순 리밸런싱 빈도 축소나 benchmark 기반 노출 축소만으로는 MDD -20% 게이트를 안정적으로 넘지 못했다. 다음은 새 `target_weight_downside_rank_relief` 후보군을 top-200 full sweep으로 검증해 종목 선별 단계의 낙폭·하방변동성 페널티가 MDD와 위험조정 alpha를 개선하는지 확인한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하되, top-200 후속 결과만으로 신규 paper/live 전환은 진행하지 않는다 |
