...
| ✅ **Paper 운영 도구** | `tools/` — evidence pipeline, pilot control, bootstrap, preflight, launch readiness CLI. `tools/paper_pilot_control.py`와 `tools/paper_runtime_status.py`는 액션 옵션을 한 번에 하나만 받으며, 상충되는 액션이 같이 들어오면 상태 조회·파일 기록 전에 fail-closed로 종료한다 |
...
| ✅ **paper preflight/readiness 대상 옵션 충돌 차단** | `tools/paper_preflight.py`와 `tools/paper_launch_readiness.py`의 운영 대상 범위를 `--strategy` 또는 `--all` 중 하나로 제한한다. 두 옵션이 함께 들어오면 DB 초기화, readiness 계산, artifact 기록 전에 argparse 단계에서 실패해 단일 전략 점검이 전체 전략 점검으로 조용히 바뀌지 않는다 |
...
| ✅ **paper 체결 품질 리포트 추가** | `tools/paper_trade_quality_report.py`가 `TradeHistory`의 `expected_price`, `price_gap`, `actual_slippage_pct`, `slippage`를 집계해 예상가 대비 불리한 체결 갭, expected_price 누락, 종목/일자별 체결 품질을 JSON/Markdown으로 남긴다. `generate_promotion_package()`도 같은 기간의 `trade_quality`를 기록하고 expected_price 누락 또는 50bp 초과 불리한 체결 갭이 있으면 `BLOCKED` 처리해 paper evidence가 실제 체결 왜곡을 감당하는지 승격 전 확인할 수 있다 |
