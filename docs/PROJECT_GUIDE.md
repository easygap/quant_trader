...
| ✅ **paper 체결 품질 리포트 추가** | `tools/paper_trade_quality_report.py`가 `TradeHistory`의 `expected_price`, `price_gap`, `actual_slippage_pct`, `slippage`를 집계해 예상가 대비 불리한 체결 갭, expected_price 누락, 종목/일자별 체결 품질을 JSON/Markdown으로 남긴다. 기본 CLI는 `review`와 `no_trades`를 non-zero로 종료하며, 단순 리포트 목적의 체결 0건은 `--allow-no-trades`로만 성공 처리한다. `generate_promotion_package()`도 같은 기간의 `trade_quality`를 기록하고 no_trades, expected_price 누락 또는 50bp 초과 불리한 체결 갭이 있으면 `BLOCKED` 처리해 paper evidence가 실제 체결 왜곡을 감당하는지 승격 전 확인할 수 있다 |
