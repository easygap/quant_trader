...
| ✅ **target-weight 변동성 타깃 후보군 추가** | `benchmark_vol_target` 노출 모드를 추가해 benchmark 실현 변동성이 목표치를 초과하면 base exposure를 비례 축소하고, benchmark drawdown trigger가 걸리면 지정 floor까지 낮춘다. `target_weight_volatility_target` family로 top-5 hold-buffer/risk-overlay/tolerance 후보의 변동성 타깃 변형을 따로 검증한다 |
...
| 다음 후보 탐색 | top-200 저회전 sweep에서 회전율은 낮아졌지만 benchmark excess Sharpe와 MDD 병목이 유지됨. 다음은 새 `target_weight_volatility_target` 후보군을 top-200 full sweep으로 검증해 변동성 타깃과 낙폭 floor가 MDD와 위험조정 alpha를 실제로 개선하는지 확인한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하되, top-200 리스크 완화/저회전 결과만으로 신규 paper/live 전환은 진행하지 않는다 |
