...
| 다음 후보 탐색 | next-open canonical 재검증은 완료. 다음은 `--shadow-days 3`로 고유 resolved trade_day 기준 shadow/dry-run evidence 3 clean days 확보 → `--readiness-audit`로 launch/cap/auth/position/idempotency/장 시간/가격 최신성/liquidity/pre-trade risk와 stale valuation 빈도 점검 → `paper_pilot_control.py --enable`로 requested cap 재감사 후 승인 → 정규장 중 capped paper 순서로 execution-backed evidence 품질 검증 |
