...
| 다음 후보 탐색 | 새 알파 탐색보다 target-weight pilot을 `--shadow-days 3`로 고유 resolved trade_day 기준 shadow/dry-run evidence 3 clean days 확보 → `--readiness-audit`로 launch/cap/auth/position/idempotency/liquidity/pre-trade risk 점검 → cap 조정 승인 → capped paper 순서로 돌려 execution-backed evidence 품질을 검증 |
