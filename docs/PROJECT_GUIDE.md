...
| **portfolio_backtester.py** | 다종목 동시 보유 시뮬레이션. 분산·최대 포지션·총투자비중 한도와 함께 종목별 OHLCV/event column 기반 gap-up 매수 차단, gap-down 청산, 어닝 윈도우 매수 차단, BlackSwan 청산·쿨다운·recovery 사이징을 반영하고 `gap_*`/`earnings_*`/`blackswan_*` 진단 카운터를 반환. |
