...
| ✅ **target-weight research 다음 시가 체결 보강** | 직전 거래일 점수로 선정한 target-weight 리밸런싱을 당일 종가가 아니라 다음 거래일 원본 `open`으로 체결하고, 종가로 일말 평가한다. 리밸런싱일 시가 누락은 `target_weight_research_execution_price_missing`으로 차단하고 기존 종가 체결 기반 research artifact는 재생성 또는 `execution_price_mode` 확인 후 사용한다 |
...
| 다음 후보 탐색 | 먼저 next-open 리서치 체결 기준으로 target-weight 후보 sweep/canonical 재검증 → `--shadow-days 3`로 고유 resolved trade_day 기준 shadow/dry-run evidence 3 clean days 확보 → `--readiness-audit`로 launch/cap/auth/position/idempotency/장 시간/가격 최신성/liquidity/pre-trade risk 점검 → `paper_pilot_control.py --enable`로 requested cap 재감사 후 승인 → 정규장 중 capped paper 순서로 돌려 execution-backed evidence 품질을 검증 |
