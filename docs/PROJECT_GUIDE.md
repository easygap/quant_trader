...
| **liquidate_trigger.py** | `LIQUIDATE_TRIGGER_TOKEN`·`LIQUIDATE_TRIGGER_PORT` 설정 시 POST /liquidate (X-Token 또는 ?token=)으로 긴급 청산. live 설정에서는 `ENABLE_LIVE_TRADING=true`와 `LIQUIDATE_TRIGGER_CONFIRM_LIVE=true`가 함께 필요하며, 내부 guard 실패는 HTTP 실패 응답으로 반환한다. |
...
| **HTTP** | `LIQUIDATE_TRIGGER_TOKEN`·`LIQUIDATE_TRIGGER_PORT` 설정 후 `python -m monitoring.liquidate_trigger`, POST /liquidate. live 설정에서는 `ENABLE_LIVE_TRADING=true`와 `LIQUIDATE_TRIGGER_CONFIRM_LIVE=true`를 함께 요구하며, 확인 실패는 서버 종료 대신 실패 응답으로 반환한다. |
