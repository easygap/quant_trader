...
| ✅ **promotion blocker summary 추가** | `tools/evaluate_and_promote.py --canonical`이 `promotion_blocker_summary.json/md`를 함께 생성하고, `--blocker-summary`로 기존 `reports/promotion` artifact만 읽어 요약을 재생성할 수 있다. 요약에는 source artifact hash를 남기며 `--blocker-summary-check`로 `promotion_result/metrics/run_metadata`와 동기화됐는지 검증한다. 전략별 status, 주요 blocker, 핵심 metrics, 다음 운영 조치를 한 화면에 정리해 긴 `promotion_result.reason`을 직접 파싱하지 않아도 된다 |
