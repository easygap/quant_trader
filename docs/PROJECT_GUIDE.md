...
8. `reports/paper_evidence/promotion_evidence_{strategy}.json` recommendation이 `ELIGIBLE`이어야 하며, 60영업일 execution-backed evidence, benchmark_final_ratio 80% 이상, 양의 excess/cumulative return, 최소 sell 5건, win_rate 45% 이상, frozen day 0, 최신 evidence date 기준 14일 이내 freshness를 만족해야 함
...
| ✅ **paper 체결 품질 리포트 추가** | `tools/paper_trade_quality_report.py`가 `TradeHistory`의 `expected_price`, `price_gap`, `actual_slippage_pct`, `slippage`를 집계해 예상가 대비 불리한 체결 갭, expected_price 누락, 종목/일자별 체결 품질을 JSON/Markdown으로 남긴다. 승격 판단 전에 paper evidence가 실제 체결 왜곡을 감당하는지 운영자가 확인할 수 있다 |
