...
| ✅ **research empty-universe fail-closed 추가** | strict 유동성 필터나 데이터 수집 실패로 research universe가 0개가 되면 benchmark/candidate 평가를 건너뛰고 `INSUFFICIENT_BENCHMARK_DATA`, `empty_universe_reason`, `skipped_due_to_data`를 artifact와 Markdown에 남겨 0% 성과 후보를 실제 검증 결과로 오해하지 않게 했다 |
