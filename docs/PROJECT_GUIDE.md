...
운영 점검: `python tools/evaluate_and_promote.py --check-only`는 promotion artifact 로드뿐 아니라 `metrics_summary.json`의 WF/benchmark 파생값 동기화, `promotion_result.json` 재계산, `promotion_blocker_summary.json`, `reports/current_blockers.json` 동기화까지 함께 검증한다.
