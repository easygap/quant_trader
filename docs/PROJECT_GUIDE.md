...
| 다음 후보 탐색 | top-200 저회전 sweep에서 회전율은 낮아졌지만 benchmark excess Sharpe와 MDD 병목이 유지됨. 다음 후보는 단순 리밸런싱 빈도 조정보다 변동성 타깃, 낙폭 차단, 위험예산 기반 포지션 크기 산식처럼 손실 구간의 노출을 더 직접 낮추는 방향을 우선 검증한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하되, top-200 리스크 완화/저회전 결과만으로 신규 paper/live 전환은 진행하지 않는다 |
