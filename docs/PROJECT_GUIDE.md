...
| **tools/research_candidate_sweep.py** | promotion/live artifact와 분리된 research-only 후보 공장. `--candidate-family rotation|momentum|breakout|pullback|benchmark_relative|risk_budget|cash_switch|all` 후보를 포트폴리오 단위로 평가하고 benchmark excess, EV, CAGR, turnover, WF 안정성으로 랭킹하며 decision action을 포함해 `reports/research_sweeps/`에 저장. |
...
| ✅ **Research candidate sweep** | `tools/research_candidate_sweep.py` — rotation/momentum/breakout/pullback/benchmark-relative/risk-budget/cash-switch 후보군을 benchmark-aware artifact로 랭킹하고 decision action 생성. promotion/live gate와 분리 |
...
| 현재 후보군 | rotation/momentum/breakout 기존 14개 모두 research_only 유지. pullback 4개, benchmark-relative 3개, risk-budget 5개, cash-switch 3개도 research-only 유지 |
| 다음 후보 탐색 | risk-budget/cash-switch 모두 excess gate 실패. 다음은 단순 방어보다 동일 유니버스 B&H를 직접 이길 수 있는 benchmark-aware 랭킹, exposure-matched benchmark, 부분 헤지 설계 우선 |
