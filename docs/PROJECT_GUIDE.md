...
| 다음 후보 탐색 | top-200 리스크 완화·저회전·변동성 타깃 sweep을 모두 확인한 결과, 단순 리밸런싱 빈도 축소나 benchmark 기반 노출 축소만으로는 MDD -20% 게이트를 안정적으로 넘지 못했다. 다음은 목표 노출 산식이 아니라 종목 선별 랭킹 단계에 낙폭·하방변동성·상관/업종 집중 페널티를 반영해 고위험 추격 매수를 줄이는 후보군을 설계한다. 기존 canonical provisional 후보의 paper pilot은 별도 운영 단계로 유지하되, top-200 후속 결과만으로 신규 paper/live 전환은 진행하지 않는다 |
