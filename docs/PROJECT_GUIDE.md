...
| ✅ **target-weight shadow proof 추가** | dry-run에서 `--record-shadow-evidence`를 켜면 `append_shadow_plan_evidence()`가 non-promotable `shadow_bootstrap` record를 남김. `execution_backed=False`, excess=null이라 promotion은 오염하지 않고, launch readiness의 clean final day만 채운다. `--shadow-start-date/--shadow-end-date`로 여러 날짜 shadow bootstrap을 한 번에 누적할 수 있고, 같은 실행에서 launch readiness JSON/MD와 pilot runbook을 생성하며 session/batch artifact에는 기본 cap preview, plan 기반 최소/추천 cap, enable 명령, launch artifact 경로를 기록 |
...
| 다음 후보 탐색 | 새 알파 탐색보다 target-weight pilot을 `--shadow-start-date/--shadow-end-date`로 shadow/dry-run evidence 3 clean days 확보 → cap 조정 승인 → capped paper 순서로 돌려 execution-backed evidence 품질을 검증 |
