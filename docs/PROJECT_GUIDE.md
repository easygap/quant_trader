...
| ✅ **DB 복구 guarded apply 추가** | `--apply-db-restore --restore-apply-plan <apply_plan_json>`은 백업 확인과 명시 적용 확인 플래그가 모두 있어야 실제 DB write를 수행한다. 실행 직전 verification/CSV/current DB 상태를 다시 확인하고 기존 trade/position이 있으면 차단하며, 통과 시 trade_history와 positions를 단일 트랜잭션으로 반영하고 결과 artifact를 남긴다. ready apply plan이 있으면 current blockers/status가 guarded apply 명령을 다음 단계로 안내한다 |
