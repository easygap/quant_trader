...
| **target_weight_rotation.py** | Target-weight 후보의 portfolio-level plan 생성. 직전 거래일 점수, KS11 risk overlay, 목표비중 수량 산출, pilot cap 검증, 주문 종목별 최근 평균 거래대금/기존 평균매입가 diagnostics를 담당. 일반 `generate_signal()` 전략으로 위장하지 않는다. 전용 pilot 실행은 주문 실패, 유동성 preflight 실패, 비용 반영 pre-trade risk 실패 시 후속 주문을 중단한다. `tools/target_weight_rotation_pilot.py --record-shadow-evidence`로 dry-run launch-readiness evidence, launch readiness JSON/MD, pilot runbook을 남기고, cap preview·평균 거래대금 대비 주문 비율·비용 반영 현금/비중 한도로 pilot 승인 전 기본/제안 캡 및 실행 적합성을 확인한다. |
