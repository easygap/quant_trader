...
def test_current_blockers_regenerates_stale_db_restore_review_bundle(tmp_path):
    from tools.evaluate_and_promote import build_current_blockers_report

    manifest_path = tmp_path / "restore_manifest.json"
    manifest_path.write_text(
        json.dumps({"artifact_type": "target_weight_db_restore_candidate_package"}),
        encoding="utf-8",
    )
    review_dir = tmp_path / "review_bundle"
    review_dir.mkdir()
    trade_template = review_dir / "reviewed_authoritative_trade_history.csv"
    positions_template = review_dir / "reviewed_authoritative_positions.csv"
    trade_template.write_text("", encoding="utf-8")
    positions_template.write_text("", encoding="utf-8")
    blocker_summary = {
        "artifact_type": "promotion_blocker_summary",
        "schema_version": 1,
        "generated_at": "2026-05-13T14:07:37",
    ...
    latest_snapshot_diagnostics = {
        ...
        "db_restore_candidate_package": {
            ...
            "trade_history_candidate_rows": 1,
            "position_candidate_rows": 1,
            "requires_authoritative_confirmation": True,
        },
        "operator_commands": {
            "verify_db_restore_package": (
                "python tools/target_weight_rotation_pilot.py "
                f"--verify-db-restore-package --restore-manifest {manifest_path.as_posix()}"
            )
        },
    }
    latest_db_restore_verification = {
        ...
        "status": "blocked",
        "restore_ready": False,
        "blockers": ["authoritative_trade_history_csv_empty_template"],
        "candidate_package": {
            "trade_history": {"hash_ok": True},
            "positions": {"hash_ok": True},
        },
        "authoritative_evidence": {
            "trade_history": {"provided": True, "match": False},
            "positions": {"provided": True, "match": False},
        },
    }
    latest_db_restore_review_bundle = {
        "artifact_type": "target_weight_db_restore_review_bundle",
    ...
    action = report["next_actions"][0]
    assert action["snapshot_db_restore_review_bundle_manifest_stale"] is True
    assert action["snapshot_db_restore_review_bundle_manifest_hash"] == "0" * 64
    assert action["snapshot_db_restore_review_bundle_current_manifest_hash"]
    assert action["command"] == (
        "python tools/target_weight_rotation_pilot.py "
        f"--prepare-db-restore-review-bundle --restore-manifest {manifest_path.as_posix()}"
    )
    assert action["scheduled_command"] == action["command"]
    assert action["requires"] == "fresh DB restore review bundle"
    assert (
        action["db_restore_review_guard"]
...
def test_current_blockers_requires_reverify_when_ready_restore_csv_becomes_stale(tmp_path):
    from tools.evaluate_and_promote import build_current_blockers_report
    import tools.evaluate_and_promote as ep
    ...
    with trade_template.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        writer.writeheader()
        writer.writerow({**trade_row, "price": "70001"})
    position_row = {
        "account_key": "target_weight_best",
        "symbol": "005930",
        "quantity": "1",
    ...
    for path in (candidate_positions, positions_template):
        with path.open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(
                handle,
                fieldnames=ep.TARGET_WEIGHT_RESTORE_POSITION_COMPARE_COLUMNS,
            )
            writer.writeheader()
    ...
    latest_snapshot_diagnostics = {
        ...
        "db_restore_checklist": {
            ...
            "positions": {
                ...
                "current_db_position_count": 0,
                "missing_or_unverified_symbols": ["005930"],
            },
        },
        "db_restore_candidate_package": {
            "generated": True,
            "manifest_path": "reports/paper_runtime/restore_manifest.json",
            "trade_history_candidate_csv": candidate_trade.as_posix(),
            "positions_candidate_csv": candidate_positions.as_posix(),
            "trade_history_candidate_rows": 1,
            "position_candidate_rows": 1,
            "requires_authoritative_confirmation": True,
        },
    ...
    action = report["next_actions"][0]
    assert action["snapshot_db_restore_verification_ready"] is True
    assert action["snapshot_db_restore_verification_stale_after_review_edit"] is True
    assert action["command"] == (
        "# blocked: reviewed authoritative DB restore verification stale after CSV edit"
    )
    assert action["scheduled_command"] == action[
        "snapshot_db_restore_verify_after_manual_review_command"
    ]
    assert action["requires"] == "rerun DB restore verification after reviewed CSV edit"
    assert (
        action["db_restore_review_guard"]
        == "target_weight_authoritative_db_restore_verification_stale_after_review_edit"
    )
    assert "manual DB restore" not in action["requires"]


def test_current_blockers_routes_restore_ready_to_no_write_apply_plan():
    from tools.evaluate_and_promote import build_current_blockers_report
    ...
    assert action["scheduled_command"] == apply_plan_command
    assert action["requires"] == "no-write DB restore apply plan"
    assert (
        action["db_restore_review_guard"]
...
def test_current_blockers_routes_ready_apply_plan_to_pre_apply_backup():
    ...
    apply_command = (
        ...
        "--restore-apply-plan reports/paper_runtime/apply_plan.json "
        "--restore-backup reports/paper_runtime/backup.json "
    ...
    latest_db_restore_apply_plan = {
        ...
        "operator_commands": {
            ...
            "backup_db_restore_state": backup_command,
        },
    }
    ...
    assert action["snapshot_db_restore_backup_command"] == backup_command
    assert action["command"] == backup_command
    assert action["scheduled_command"] == backup_command
    assert action["requires"] == "pre-apply DB restore backup artifact"
    assert (
        action["db_restore_review_guard"]
...
def test_current_blockers_routes_ready_backup_to_guarded_apply():
    from tools.evaluate_and_promote import build_current_blockers_report
    ...
    latest_db_restore_apply_plan = {
        ...
        "blockers": [],
        "operator_commands": {
            "apply_manual_db_restore": "# blocked: use backup-scoped apply command",
            "backup_db_restore_state": (
                "python tools/target_weight_rotation_pilot.py --backup-db-restore-state "
                "--restore-apply-plan reports/paper_runtime/apply_plan.json"
            ),
        },
    ...
    latest_db_restore_backup = {
        ...
        "source_path": "reports/paper_runtime/backup.json",
        "candidate_id": "target_weight_best",
        "snapshot_date": "2026-05-20",
        ...
        "blockers": [],
        "operator_commands": {
            "apply_guarded_db_restore": apply_command,
        },
    }
    ...
    action = report["next_actions"][0]
    assert action["snapshot_db_restore_apply_plan_ready"] is True
    assert action["snapshot_db_restore_backup_ready"] is True
    assert action["snapshot_db_restore_backup_apply_command"] == apply_command
