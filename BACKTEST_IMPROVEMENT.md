...
| target-weight portfolio drawdown guard 후보군 | 높음 | 후보군 추가 — `target_weight_drawdown_guard` family가 직전 평가 NAV 기준 포트폴리오 drawdown trigger, 목표 노출 floor, 재진입 cooldown을 적용한다. 다음 검증은 `--candidate-family target_weight_drawdown_guard --top-n 200` full sweep으로 MDD 완화와 alpha 훼손 여부를 확인 |
