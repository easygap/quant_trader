...
| target-weight portfolio drawdown guard 후보군 | 높음 | canonical 검증 완료 — `target_weight_drawdown_guard` family에 exp75/rankrisk90/pdd10/floor40 기반 `tol3`, `tol4`, `tol5`, `maxnew2`, `tol3_maxnew2` 변형을 추가하고 `--top-n 200` full sweep에서 후보 10개 비교. 새 `tol4` 후보는 research sweep에서 return +81.19%, raw excess +49.30%p, MDD -19.33%, turnover/year 886.7%, WF positive/Sh+ 100%로 canonical 평가 대상에 포함됐다. canonical 재평가에서는 pdd8/floor25 후보가 benchmark excess -6.46%p로 `paper_only`, exp75 원본이 turnover/year 1026.3%로 `paper_only`, exp75 tol3 후보도 turnover/year 1009.0%로 `paper_only`, exp75 tol4 후보는 return +183.02%, benchmark excess +33.63%p, Sharpe 1.50, PF 4.82, turnover/year 986.5%로 회전율은 통과했지만 MDD -20.25%로 `paper_only`. 기존 risk-overlay 후보만 provisional 유지. 다음은 tol4의 회전율 개선을 유지하면서 drawdown floor/cooldown 또는 ranking risk penalty를 보수화해 MDD를 -20% 안쪽으로 낮추는 검증 |
