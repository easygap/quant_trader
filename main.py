...
예시:
  ...
  python main.py --mode dashboard  # 실시간 웹 대시보드 (기본 http://127.0.0.1:8080)
    ...
    parser.add_argument(
        ...
        help="실행 모드. backtest_momentum_top: 모멘텀 상위 동일비중 멀티종목. portfolio_backtest: 멀티종목 포트폴리오 백테스트. paper: 워치리스트 1회. schedule: 모의 스케줄 무한 루프(상시 서버). rebalance: 바스켓 리밸런싱. deploy_check: 바스켓 배포 점검(계획·비용·활성화 절차).",
    )
