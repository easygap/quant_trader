...
def _target_weight_effective_execution_status(
    ...
    *,
    ...
    if (
        status == "READY_TO_EXECUTE"
        and execute_command
        and _command_is_blocked(execute_command)
    ):
        reason = str(execute_command).lstrip()[len("# blocked:"):].strip()
        return "BLOCKED", reason or "daily ops READY_TO_EXECUTE command blocked"
    ...
    if (
        ...
        if (
            ...
            if priority_scheduled_command:
                return (
                    "PREPARE no-write DB restore apply plan before manual "
                    f"DB restore: {priority_scheduled_command}"
                )
        ...
        if (
            ...
            return "REGENERATE DB restore review bundle before filling CSV"
        if (
            priority_db_restore_review_guard
        ...
        if (
            ...
            if priority_scheduled_command:
                return (
                    "APPLY guarded DB restore after confirming backup: "
                    f"{priority_scheduled_command}"
                )
            return "APPLY guarded DB restore after confirming backup"
        if (
            priority_db_restore_review_guard
...
def _print_target_weight_daily_ops_status(
    ...
    *,
    ...
    freshness_blocker = _target_weight_actionable_daily_ops_freshness_blocker(summary)
    if freshness_blocker:
        print(
            "    Freshness warning: "
            + freshness_blocker.replace("# blocked: ", "", 1)
        )
