...
2026-04-30 canonical top-20 target-weight full sweep: 기존 `target_weight_rotation_top3_40_100_excess`는 return=+212.21%, raw excess=+62.82%p, exposure-matched excess=+83.66%p로 alpha 후보가 되었지만 turnover/year=1412.1% 때문에 `KEEP_RESEARCH_ONLY`. score-floor 및 turnover-aware 후보 추가 후 best=`target_weight_rotation_top5_60_120_floor0_hold3`는 return=+278.57%, raw excess=+129.18%p, exposure-matched excess=+150.88%p, Sharpe=1.65, WF positive/Sh+ 100%, turnover/year=807.8%로 turnover 병목을 통과했습니다. 다만 MDD=-28.25%로 provisional gate에는 아직 못 들어가므로 다음 개발 방향은 신규 진입 신호 추가가 아니라 drawdown-aware exposure/market-risk overlay입니다.
