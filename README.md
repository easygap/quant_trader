...
* `tools/` — Paper 운영 도구와 research sweep (evidence pipeline, pilot control, bootstrap, preflight, launch readiness, candidate sweep)
* `api/` — KIS REST·웹소켓
...
2026-04-30 canonical top-20 target-weight full sweep: 기존 `target_weight_rotation_top3_40_100_excess`는 return=+212.21%, raw excess=+62.82%p, exposure-matched excess=+83.66%p로 alpha 후보가 되었지만 promotion gate는 `paper_only`와 turnover/year=1412.1% 때문에 `KEEP_RESEARCH_ONLY`. 이어서 score-floor 후보를 추가했고, best=`target_weight_rotation_top5_60_120_floor0`는 return=+210.21%, raw excess=+60.82%p, exposure-matched excess=+83.58%p, Sharpe=1.41, WF positive=100%였으나 turnover/year=1081.5%로 아직 승격 금지. 다음 개발 방향은 신규 진입 신호 추가보다 turnover-aware rebalance/tolerance/correlation 필터입니다.
...
Paper 운영 도구: `tools/run_paper_evidence_pipeline.py` (backfill/finalize/package), `tools/paper_preflight.py`, `tools/paper_launch_readiness.py`, `tools/paper_pilot_control.py`, `tools/research_candidate_sweep.py`.
