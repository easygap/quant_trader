...
> - Research sweep: 기존 top-20 all-family 후보 14개 재검증도 `NO_ALPHA_CANDIDATE`; `pullback`, benchmark-relative momentum, risk-budget 후보군을 research-only로 추가
...
Research candidate sweep — `tools/research_candidate_sweep.py --quick --candidate-family all`로 promotion과 분리된 rotation/momentum/breakout/pullback/benchmark-relative/risk-budget 후보 랭킹 artifact를 생성.
